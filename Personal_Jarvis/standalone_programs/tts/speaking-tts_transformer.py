...
# load the processor
print("load the processor")
processor = SpeechT5Processor.from_pretrained("microsoft/speecht5_tts")
# load the model
print("load the model")
model = SpeechT5ForTextToSpeech.from_pretrained("microsoft/speecht5_tts").to(device)
# load the vocoder, that is the voice encoder
print("load the vocoder, that is the voice encoder")
...
def save_text_to_speech(text, speaker=None):
    ...
    if speaker is not None:
        ...
        output_filename = f"output_wav_files\{speaker}-transformer-voice.mp3"
    else:
        # if we don't have a speaker, we use a random string in the filename
        random_str = ''.join(random.sample(string.ascii_letters+string.digits, k=5))
        output_filename = f"output_wav_files\{random_str}-transformer-voice.mp3"
...
save_text_to_speech("Python is my favorite programming language", speaker=speakers["jmk"])
